...
def main():
    ...
    for _ in range(n):
        ...
        # table = blenderproc.loader.load_obj("/home/avena/software/items3/scenes/Box_stol.obj")[0]
        table.enable_rigidbody(False, collision_shape='CONVEX_HULL', collision_margin=0.001, mass=5)
        texture = np.random.choice(blenderproc.loader.load_texture("/home/avena/software/items3/scenes/textures"), 1)[0]
        ...
        # table.add_displace_modifier(texture=texture[0], strength=random.gauss(0, 0.5), subdiv_level=random.randint(1, 3))
        ...
        if scenario_number == 0:
            number_of_items = np.random.randint(19, 20)
        ...
        if scenario_number == 2:
            pass
        blenderproc.material.add_alpha_channel_to_textures(blurry_edges=True)


        light_p = blenderproc.types.Light()
        light_p.set_type("SPOT")
        light_p.set_location([0, 0, 1.5])
        ...
        blenderproc.camera.set_intrinsics_from_blender_params(lens=0.017453, image_width=1000, image_height=1350,
                                                              lens_unit='FOV')
        ...
        #         blenderproc.material.add_dust(material, strength=0.9)
